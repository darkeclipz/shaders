...
void main() {
    ...
    for(float aay=0.; aay < AA; aay++)
    {
        ...
        float an = 0.;//10.0*(0.94+sin(time/16.)*0.02);
        
        vec3 ta = vec3(-3.0 - 5.*(-cos(iTime/30.)*.5+.5), 0.5  ,7.);   
        an = 0.68;//0.8 + 0.21*(-cos(time / 16.)*0.5+0.5) - 0.1 ;
        
        vec3 rot = vec3(1.0*sin(an), 0., 1.0*cos(-an));
        vec3 ro = ta + vec3(0.,0.,-20.) * rot;
        ...
        if( tm.y>0.0 )
        {
            ...
            vec3 sun_dir = normalize( ro-pos-vec3(-0.65,-0.25,-0.9) );
            ...
            col += mate*vec3(0.3,0.2,0.0)*sky_dif;
            col += mate*vec3(0.5,0.4,0.4)*bou_dif;
        }
