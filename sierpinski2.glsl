...
void sphereFold(inout vec3 z, inout float dz) {
    ...
    float fixedRadius2 = 0.9 + (sin(iTime/10.)*0.5+0.5)*0.1;
    float minRadius2 = 0.5 + (sin(iTime/8.)*0.5+0.5)*0.1;
	float r2 = dot(z,z);
	if (r2< minRadius2) {
...
float DE(vec3 z)
{
    vec3 orig = vec3(z);
    float Iterations = 20.;
    float Scale = 2. - (sin(iTime/16.) * 0.5 + 0.5) * .6;
    float Offset = 1. + (sin(2.*iTime/9.) * 0.5 + 0.5)*0.05;
    float n = 0.;
    float dr = 1.;
    while (n < Iterations) {
    ...
    return (length(z)/abs(dr) ) * pow(Scale, -float(n));
}
...
vec2 map( in vec3 pos, float time ) 
{
    ...
    float d2 = DE( pos * rotateX((0.90+0.002)*3.14*2.) * rotateZ(3.14/4.) - vec3(0,-0.73,0) );
    return vec2(d2, 1.);
    if(d2 < d) id = 1.;
    d = min(d2, d);
...
vec3 camPos(float time) 
{
    ...
    vec3 p1 = vec3(-1,0.3,0.6);
    vec3 p2 = -vec3(0,1,1.2); 
    vec3 p3 = vec3(0,-1.5, 0.4); 
    vec3 p4 = vec3(0.,-1., 0.3);  
    vec3 pl = vec3(0,0.5,.9);
...
void main() {
    ...
    for(float aay=0.; aay < AA; aay++)
    {
        ...
        if( tm.x < 20. )
        {
            ...
            vec3 sun_dir = normalize( ro - vec3(0,1,0));
